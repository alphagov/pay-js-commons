...
describe('Axios base client', () => {
  ...
  describe('Manage when failing to connect to the API', () => {
    ...
    it('should call call the error hook when the status code is 600', async () => {
      nock(baseUrl)
        .get('/')
        .reply(600, {
          code: 'a code'
      ...
      expect(requestFailureSpy.mock.calls[0][0]).toEqual({
        ...
        url: '/',
        description: 'foo',
        message: 'Request failed with status code 600',
        additionalLoggingFields: { foo: 'bar' },
        code: 'a code',
        status: 600
      })
